...
export default function TradeDetail({ tradeId, onBack }) {
    ...
    const isReceiver = trade.receiver._id === userId;
    ...
    return (
        <div style={{ padding: "2rem", display: 'flex', flexDirection: 'column', height: '100%', boxSizing: 'border-box' }}>
            ...
            <h2 style={{ marginBottom: '0.5rem' }}>Negotiation with {partner.username}</h2>
            <p style={{ marginBottom: '1rem', fontWeight: 'bold', color: trade.status === 'accepted' ? 'green' : COLOR_ACCENT }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '2rem', flexGrow: 1, minHeight: '500px' }}>
                ...
                <div style={{ background: COLOR_PRIMARY_DARK, padding: '1.5rem', borderRadius: '12px', overflowY: 'auto' }}>
                    <h3>Trade Offer</h3>
                    <p style={{ opacity: 0.8, borderBottom: '1px solid #444', paddingBottom: '0.5rem' }}>
                        *This side can be made interactive for updating items (omitted here for brevity).
                    </p>
